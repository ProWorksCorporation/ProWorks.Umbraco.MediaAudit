...
- How does the system treat a media *folder* that contains only unused children — is the folder itself flagged, or only leaf files? **Resolved**: only leaf files are classified/listed — a folder is an organizational container, not an audited item, so it's excluded from results entirely rather than always showing as "Unused" clutter.
- What happens when a content property references a media *folder itself* (e.g. a gallery/slideshow that picks a folder and renders whatever's inside it) rather than each child file individually? Umbraco records the relation on the folder node, not on its children. **Resolved**: a file MUST be classified "Used" if a folder it lives in (any ancestor, not just the immediate parent) is itself referenced — otherwise every file in such a folder would be misclassified "Unused" and become eligible for deletion despite being genuinely rendered on the site. This propagation is applied everywhere usage is decided: bulk classification, usage-detail lookup, and FR-014's pre-delete re-check (which reuses the usage-detail lookup directly, so the fix can't drift out of sync between the two). FR-018's purge only re-checks `Trashed` state, not usage, since a purge target is already confirmed-deleted media.
