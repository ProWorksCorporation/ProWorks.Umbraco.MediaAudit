...
- Server (C#): `src/ProWorks.Umbraco.MediaAudit/`
- Backoffice client (TypeScript/Lit/Vite): `src/ProWorks.Umbraco.MediaAudit/Client/`
- Sample site for local dev/validation: `src/ProWorks.Umbraco.MediaAudit.Web/`
- Tests: `tests/ProWorks.Umbraco.MediaAudit.Tests.Unit/`, `tests/ProWorks.Umbraco.MediaAudit.Tests.Integration/`, `tests/ProWorks.Umbraco.MediaAudit.Client.Tests/`
...
- [X] T001 Create the repository project structure per plan.md's Project Structure: `src/ProWorks.Umbraco.MediaAudit/` (RCL, with `Composers/`, `Constants.cs`, `Controllers/`, `Migrations/`, `Services/`, `Models/`, `Client/`, `wwwroot/` subfolders) and `src/ProWorks.Umbraco.MediaAudit.Web/`
- [X] T002 [P] Initialize Vite + TypeScript + Lit config in `src/ProWorks.Umbraco.MediaAudit/Client/` — `package.json`, `tsconfig.json` (with `@umbraco-cms/backoffice/extension-types` in `compilerOptions.lib`), and `vite.config.ts` (`outDir: "wwwroot"`, `base: "/App_Plugins/ProWorks.Umbraco.MediaAudit/"`, `@umbraco-cms/*` externalized) per research.md §2
- [X] T003 [P] Create the extension manifest skeleton `src/ProWorks.Umbraco.MediaAudit/Client/public/umbraco-package.json` (empty `extensions` array, package `name`/`version`) per research.md §2
- [X] T004 [P] Configure `src/ProWorks.Umbraco.MediaAudit/ProWorks.Umbraco.MediaAudit.csproj` with Marketplace metadata (`Title`, `Description`, `Version`, `Authors`, `PackageProjectUrl`, `PackageLicenseExpression`) and the `umbraco-marketplace` NuGet tag per research.md §2
- [X] T005 [P] Scaffold `src/ProWorks.Umbraco.MediaAudit.Web/` as a standard Umbraco v17 template site that project-references `ProWorks.Umbraco.MediaAudit`, for local dev and quickstart.md validation
- [X] T006 [P] Scaffold xUnit test projects `tests/ProWorks.Umbraco.MediaAudit.Tests.Unit/` and `tests/ProWorks.Umbraco.MediaAudit.Tests.Integration/` (the latter configured to run against a seeded local Umbraco v17 SQLite instance) per research.md §7
- [X] T007 [P] Scaffold Web Test Runner config in `tests/ProWorks.Umbraco.MediaAudit.Client.Tests/` for Lit component tests per research.md §7
...
- [X] T008 [P] Create `MediaAuditItem` model in `src/ProWorks.Umbraco.MediaAudit/Models/MediaAuditItem.cs` per data-model.md
- [X] T009 [P] Create `MediaUsageReference` model (including the `Culture` field) in `src/ProWorks.Umbraco.MediaAudit/Models/MediaUsageReference.cs` per data-model.md
- [X] T010 [P] Create `AuditRun` model in `src/ProWorks.Umbraco.MediaAudit/Models/AuditRun.cs` per data-model.md
- [X] T011 [P] Create `MediaFolder` model in `src/ProWorks.Umbraco.MediaAudit/Models/MediaFolder.cs` per data-model.md
- [X] T012 Implement `IMediaReferenceScanner`/`MediaReferenceScanner` in `src/ProWorks.Umbraco.MediaAudit/Services/MediaReferenceScanner.cs` — the scan-based safety net ported from `reference/media_audit.py`, reading via `IContent.Properties`/`Property.GetValue(culture, segment)` **once per configured language/segment** (not just the default), matching GUID with/without hyphens and file path/filename, scoped to `IContentService` only (no Member data) per research.md §4, §8, §9 (depends on T008–T011)
- [X] T013 Implement `IMediaAuditService`/`MediaAuditService` in `src/ProWorks.Umbraco.MediaAudit/Services/MediaAuditService.cs` — relation-based classification via `IRelationService`/`IDataValueReference` as the fast primary signal, with `MediaReferenceScanner` invoked for deep-scan mode and pre-delete/pre-purge re-checks, per research.md §4 (depends on T012)
- [X] T014 Implement the `MediaAuditApiController` skeleton (routing, auth gate requiring Media-section access per FR-013) with stub `GET /summary`, `POST /run`, `GET /items` actions in `src/ProWorks.Umbraco.MediaAudit/Controllers/MediaAuditApiController.cs` per contracts/media-audit-api.md (depends on T013)
- [X] T015 Implement `MediaAuditComposer.cs` in `src/ProWorks.Umbraco.MediaAudit/Composers/MediaAuditComposer.cs` registering `IMediaReferenceScanner`, `IMediaAuditService`, and the API controller in Umbraco's DI container (depends on T012–T014)
- [X] T016 [P] Create the dashboard shell element and its extension registration in `src/ProWorks.Umbraco.MediaAudit/Client/src/dashboard/media-audit-dashboard.element.ts` and `src/ProWorks.Umbraco.MediaAudit/Client/src/manifests.ts` (`type: "dashboard"` entry per research.md §3), plus `src/ProWorks.Umbraco.MediaAudit/Client/src/index.ts`
- [X] T017 [P] Create the API client wrapper `src/ProWorks.Umbraco.MediaAudit/Client/src/api/media-audit.repository.ts` covering all endpoints in contracts/media-audit-api.md
...
- [X] T022 [US1] Implement the audit progress indicator and "last refreshed" timestamp in `src/ProWorks.Umbraco.MediaAudit/Client/src/dashboard/media-audit-dashboard.element.ts` per FR-011 and User Story 1 Acceptance Scenario 3
...
- [X] T029 [P] [US2] Create `src/ProWorks.Umbraco.MediaAudit/Client/src/dashboard/media-audit-detail.element.ts` rendering the usage list (content name/type/culture/publish-state badges, navigate links) per FR-005 and User Story 2 Acceptance Scenarios 1–2
...
- [X] T038 [P] [US4] Create `AddDeletionLogTablePlan` (`PackageMigrationPlan`) in `src/ProWorks.Umbraco.MediaAudit/Migrations/AddDeletionLogTablePlan.cs` creating the `DeletionLogEntry` table per research.md §10 and data-model.md — fluent `Create.Table(...).WithColumn(...)` API (verified via reflection against the real assembly, not guessed), not an attribute-decorated DTO; confirmed running correctly against the live SQLite dev DB
- [X] T039 [P] [US4] Create the `DeletionLogEntry` model in `src/ProWorks.Umbraco.MediaAudit/Models/DeletionLogEntry.cs` per data-model.md
...
- [X] T048 [P] [US4] Create `media-audit-delete-confirm.element.ts` in `src/ProWorks.Umbraco.MediaAudit/Client/src/dashboard/` per FR-014
- [X] T049 [P] [US4] Create `media-audit-purge-confirm.element.ts` — its own, more strongly-worded confirmation, distinct from delete's — in `src/ProWorks.Umbraco.MediaAudit/Client/src/dashboard/` per FR-018 — requires ticking an explicit "I understand this is permanent" checkbox before the button enables, not just a click
- [X] T050 [P] [US4] Create `media-audit-deletion-log.element.ts` (admin-only log view) in `src/ProWorks.Umbraco.MediaAudit/Client/src/dashboard/` per FR-019 — **also where purge is initiated from**: each `Delete`-type log entry gets its own Purge action against exactly that entry's items, since "specific items previously deleted via FR-014" (FR-018's own wording) are precisely a Delete entry's item list, so no separate Recycle-Bin item-picker UI was needed
...
- [X] T054 [P] Seed a ~10,000-item media library and confirm the audit completes within 60 seconds per SC-002 — implemented as `PerformanceTests.cs` in `tests/ProWorks.Umbraco.MediaAudit.Tests.Integration/`, reusing T057's real Umbraco+SQLite harness rather than a mocked estimate. Seeds 10,000 media items via `IMediaService`'s bulk `Save(IEnumerable<IMedia>, int)` overload (one transaction, not 10,000 individual scopes - seeding itself isn't part of what SC-002 measures, so it doesn't need to be fast, just not dominate the test's wall-clock for no reason). **Measured result: the audit itself completed in 5.4 seconds** (seeding took 190s separately) - well within the 60s budget. Test asserts `TotalScanned == 10000`, `Status == Complete`, and elapsed time `<= 60s`, so a future regression fails loudly rather than silently.
- [X] T055 [P] Cross-validate audit results against `reference/media_audit.py` per quickstart.md scenario 8; confirm any discrepancies are the documented, expected ones (research.md §4, §9) and not unexplained bugs — verified for real against the actual local sample site's live database (`src/ProWorks.Umbraco.MediaAudit.Web/umbraco/Data/Umbraco.sqlite.db`), not a synthetic dataset. First confirmed the Python script's Umbraco v10-era schema assumptions (`cmsContentType`, `umbracoMediaVersion`, `umbracoPropertyData.textValue`/`varcharValue`, the `B796F64C-...` media object-type GUID) still hold unchanged in v17 - they do, so the script ran completely unmodified. Wrote `specs/001-media-usage-audit/reference/cross-validate.mjs`, a reusable Playwright script that logs into the real running backoffice, clicks Run Audit, and captures the dashboard's own authenticated `/summary` and `/items` responses (rather than reconstructing the Bearer-token auth flow by hand, which 401s per the project's own known gotcha) - then diffs every item's classification against the Python script's CSV output by id. **Result: all 9 items in the current dataset agree exactly between the two independent implementations** (5 unused: ids 1058/1060/1063/1065/1068; 4 used: ids 1059/1061/1064/1067) - zero discrepancies, not merely discrepancies that turned out to be documented/expected. The specific edge cases research.md predicts *might* diverge (Member-only reference, non-default-culture reference) aren't exercised by the current live dataset's content, so this run doesn't confirm or refute those specific predictions - re-run `cross-validate.mjs` after seeding content that exercises them for that narrower check.
- [X] T056 [P] Add xUnit unit tests for `MediaAuditService`, `MediaReferenceScanner`, `MediaDeleteService`, `MediaPurgeService`, and `DeletionLogService` in `tests/ProWorks.Umbraco.MediaAudit.Tests.Unit/` — 31 tests, all passing. Uses Moq for the service-interface boundary, but real (not mocked) Umbraco model objects (`Content`/`Media`/`ContentType`/`MediaType`/`PropertyType`) since those are plain constructible POCOs, not database-backed - verified their exact constructor/`SetValue` shapes via reflection first rather than guessing. `DeletionLogService.DeletionLogRow` was changed `private` → `internal` (+ `AssemblyInfo.cs` `InternalsVisibleTo`) purely for testability, so Moq can set up `IUmbracoDatabase`'s generic `Insert<T>`/`Fetch<T>` calls against a nameable type. Along the way, found and fixed two real (unrelated to tests) obsolete-API warnings only visible on a `--no-incremental` rebuild: `DeletionLogService` was using the obsolete `Umbraco.Cms.Core.Scoping.IScopeProvider` (now `Umbraco.Cms.Infrastructure.Scoping`), and the migration was using `MigrationBase`, which is scheduled for removal in Umbraco 18 (now `AsyncMigrationBase`).
- [X] T057 [P] Add xUnit integration tests (against the seeded local Umbraco v17 SQLite instance) covering the full `GET`/`POST` surface in `tests/ProWorks.Umbraco.MediaAudit.Tests.Integration/` — 10 tests, all passing (~6s), against a real Umbraco host + SQLite database via `Umbraco.Cms.Tests.Integration`. Design deviations/gotchas found and fixed:
  ...
  - Package migrations (`AddDeletionLogTablePlan`) don't run automatically at this boot level either - fixed with an explicit `PackageMigrationRunner.RunPendingPackageMigrations("ProWorks.Umbraco.MediaAudit")` call as an additional `[SetUp]` step.
...
- [X] T058 [P] Add Web Test Runner component tests for the dashboard, detail, delete-confirm, purge-confirm, and deletion-log elements in `tests/ProWorks.Umbraco.MediaAudit.Client.Tests/` — 30 tests, all passing (~30s). The existing scaffold was missing several things needed for it to run at all, all fixed:
  ...
  - Its `rootDir` was implicitly scoped to this package's own folder, so the tests' imports of the real element sources (`../../../../src/ProWorks.Umbraco.MediaAudit/Client/src/...`) 404'd - widened to the repo root; the `files` glob for test-*discovery* stayed CWD-relative (a different resolution rule than `rootDir`, easy to conflate).
...
Task: "Create MediaAuditItem model in src/ProWorks.Umbraco.MediaAudit/Models/MediaAuditItem.cs"
Task: "Create MediaUsageReference model in src/ProWorks.Umbraco.MediaAudit/Models/MediaUsageReference.cs"
Task: "Create AuditRun model in src/ProWorks.Umbraco.MediaAudit/Models/AuditRun.cs"
Task: "Create MediaFolder model in src/ProWorks.Umbraco.MediaAudit/Models/MediaFolder.cs"

# Once the server chain is underway, the client shell and API repository can run alongside it:
Task: "Create dashboard shell element + manifests.ts in src/ProWorks.Umbraco.MediaAudit/Client/src/dashboard/ and src/ProWorks.Umbraco.MediaAudit/Client/src/manifests.ts"
Task: "Create media-audit.repository.ts API client wrapper in src/ProWorks.Umbraco.MediaAudit/Client/src/api/"
```
...
Task: "Create AddDeletionLogTablePlan in src/ProWorks.Umbraco.MediaAudit/Migrations/AddDeletionLogTablePlan.cs"
Task: "Create DeletionLogEntry model in src/ProWorks.Umbraco.MediaAudit/Models/DeletionLogEntry.cs"
